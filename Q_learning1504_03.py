...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning1504_03'
ACTIVE_OBSERVATIONS = [
    'hour',
...
def save_learning_dashboard(
    ...
    best_so_far = np.maximum.accumulate(rolling_mean.to_numpy())

    fig, axes = plt.subplots(2, 2, figsize=(13, 8))

    axes[0, 0].plot(episodes, rewards, color='#8d99ae', alpha=0.5, linewidth=1.3, label='Episode reward')
    axes[0, 0].plot(episodes, rolling_mean, color='#1d3557', linewidth=2.3, label=f'Rolling mean ({rolling_window})')
    if training_trace.stability_episode is not None:
        axes[0, 0].axvline(training_trace.stability_episode, color='#2a9d8f', linestyle='--', linewidth=1.5, label='Estimated stability')
    ...
    axes[0, 1].plot(episodes, rolling_mean, color='#457b9d', linewidth=2.2, label='Rolling mean')
    axes[0, 1].plot(episodes, best_so_far, color='#e76f51', linewidth=2.0, label='Best rolling mean so far')
    axes[0, 1].set_title('Learning progress trend')
    axes[0, 1].set_xlabel('Episode')
    axes[0, 1].set_ylabel('Reward trend')
    axes[0, 1].grid(alpha=0.25)
    axes[0, 1].legend(loc='best')

    axes[1, 0].plot(episodes, training_trace.epsilons, color='#ff006e', linewidth=2.0)
    axes[1, 0].set_title('Adaptive epsilon schedule')
    axes[1, 0].set_xlabel('Episode')
    axes[1, 0].set_ylabel('Epsilon')
    axes[1, 0].set_ylim(0.0, 1.05)
    axes[1, 0].grid(alpha=0.25)

    deltas = np.diff(rolling_mean.to_numpy(), prepend=rolling_mean.iloc[0])
    axes[1, 1].bar(episodes, deltas, color=np.where(deltas >= 0.0, '#2a9d8f', '#e63946'), alpha=0.8)
    axes[1, 1].axhline(0.0, color='black', linewidth=1.0)
    axes[1, 1].set_title('Change in rolling mean reward')
    axes[1, 1].set_xlabel('Episode')
    axes[1, 1].set_ylabel('Delta reward')
    axes[1, 1].grid(alpha=0.2)

    fig.suptitle(f'Learning dashboard: {reward_name}', fontsize=14)
    fig.tight_layout()
    fig.savefig(output_path, dpi=160)
    plt.close(fig)


def save_strategy_dashboard(
    reward_name: str,
    fixed_runs: Sequence[PolicyRun],
    ...
    for fixed_run in fixed_runs:
        ...
        axes[0].plot(steps, fixed_slice['action_value'], linewidth=1.2, alpha=0.7, label=fixed_run.result.policy)
    axes[0].plot(steps, learned_slice['action_value'], color='#1d3557', linewidth=2.2, label='Q-learning')
    axes[0].set_ylabel('Cooling action')
    axes[0].set_title(f'Strategy dashboard: {reward_name} (first {len(learned_slice)} steps)')
    axes[0].grid(alpha=0.25)
    axes[0].legend(loc='best', ncol=2)

    for fixed_run in fixed_runs:
        fixed_slice = fixed_run.trajectory.head(horizon)
        axes[1].plot(steps, fixed_slice['grid_import_kwh'], linewidth=1.2, alpha=0.7, label=fixed_run.result.policy)
    axes[1].plot(steps, learned_slice['grid_import_kwh'], color='#e63946', linewidth=2.2, label='Q-learning')
    axes[1].set_ylabel('Grid import [kWh]')
    axes[1].grid(alpha=0.25)

    for fixed_run in fixed_runs:
        fixed_slice = fixed_run.trajectory.head(horizon)
        axes[2].plot(steps, fixed_slice['indoor_temperature'], linewidth=1.0, alpha=0.45)
    axes[2].plot(steps, learned_slice['indoor_temperature'], color='#2a9d8f', linewidth=2.0, label='Q-learning indoor temp')
    axes[2].plot(steps, learned_slice['cooling_setpoint'], color='#6d597a', linewidth=1.8, linestyle='--', label='Cooling setpoint')
    axes[2].fill_between(steps, learned_slice['cooling_setpoint'], learned_slice['indoor_temperature'], color='#ffb703', alpha=0.15, label='Gap to setpoint')
    ...
    for fixed_run in fixed_runs:
        fixed_slice = fixed_run.trajectory.head(horizon)
        axes[3].plot(steps, fixed_slice['cumulative_grid_import_kwh'], linewidth=1.2, alpha=0.75, label=fixed_run.result.policy)
    axes[3].plot(steps, learned_slice['cumulative_grid_import_kwh'], color='#264653', linewidth=2.4, label='Q-learning')
    axes[3].fill_between(steps, 0.0, learned_slice['occupant_count'], color='#f4a261', alpha=0.12, label='Occupancy level')
...
def run_experiment(
    ...
    for reward_class in reward_classes:
        print(f'Training own adaptive Q-learning with {reward_class.__name__}...', flush=True)
        ...
        if best_fixed_run.result.total_grid_import_kwh > 0.0:
            ...
            learned_run.result.savings_vs_best_fixed_pct = None
        ...
        save_learning_dashboard(
            reward_name=reward_class.__name__,
            training_trace=training_trace,
            output_path=output_dir / f'learning_dashboard_{reward_slug}.png',
        )
        save_strategy_dashboard(
            reward_name=reward_class.__name__,
            fixed_runs=baseline_runs,
            learned_run=learned_run,
            output_path=output_dir / f'strategy_dashboard_{reward_slug}.png',
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Adaptive tabular Q-learning experiment with clearer dashboards and multiple fixed baselines.'
    )
    ...
    print('Fixed strategy: constant cooling action independent of weather and occupancy.')
    ...
    for reward_name in args.reward_configs:
        reward_slug = 'energyonly' if reward_name == 'energy_only' else 'occupancyaware'
        print(f'  {args.output_dir / f"learning_trace_{reward_slug}.csv"}', flush=True)
        print(f'  {args.output_dir / f"learning_dashboard_{reward_slug}.png"}', flush=True)
        print(f'  {args.output_dir / f"strategy_dashboard_{reward_slug}.png"}', flush=True)
