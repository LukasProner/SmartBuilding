...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning1504_05'
ACTIVE_OBSERVATIONS = [
    'occupant_count',
...
def save_learning_dashboard(training_traces: dict[str, TrainingTrace], output_path: Path) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(13, 4.8))

    colors = {
        'EnergyOnlyReward': '#1d3557',
        'WeatherOccupancyReward': '#e63946',
    }

    for reward_name, trace in training_traces.items():
        rewards = np.asarray(trace.episode_rewards, dtype=float)
        episodes = np.arange(1, len(rewards) + 1)
        rolling = pd.Series(rewards).rolling(min(10, len(rewards)), min_periods=1).mean()
        axes[0].plot(episodes, rolling, linewidth=2.2, color=colors.get(reward_name, None), label=reward_name)
        if trace.stability_episode is not None:
            axes[0].axvline(trace.stability_episode, linestyle='--', linewidth=1.2, color=colors.get(reward_name, None), alpha=0.8)
        axes[1].plot(episodes, trace.epsilons, linewidth=2.0, color=colors.get(reward_name, None), label=reward_name)

    axes[0].set_title('Learning progress by reward function')
    axes[0].set_xlabel('Episode')
    axes[0].set_ylabel('Rolling cumulative reward')
...
def save_time_comparison_dashboard(fixed_run: PolicyRun, learned_runs: dict[str, PolicyRun], output_path: Path, horizon: int) -> None:
    fixed_slice = fixed_run.trajectory.head(horizon)
    steps = fixed_slice['time_step'].to_numpy()
    fig, axes = plt.subplots(3, 1, figsize=(13, 10), sharex=True)

    axes[0].plot(steps, fixed_slice['reward'], color='#6c757d', linewidth=1.8, label='Fixed strategy reward')
    for reward_name, learned_run in learned_runs.items():
        learned_slice = learned_run.trajectory.head(horizon)
        axes[0].plot(steps, learned_slice['reward'], linewidth=1.8, label=f'Q-learning reward: {reward_name}')
    axes[0].set_title(f'Reward in the same time horizon (first {len(fixed_slice)} steps)')
    axes[0].set_ylabel('Step reward')
    axes[0].grid(alpha=0.25)
    axes[0].legend(loc='best')

    axes[1].plot(steps, fixed_slice['cumulative_reward'], color='#6c757d', linewidth=2.0, label='Fixed strategy')
    for reward_name, learned_run in learned_runs.items():
        learned_slice = learned_run.trajectory.head(horizon)
        axes[1].plot(steps, learned_slice['cumulative_reward'], linewidth=2.0, label=f'Q-learning: {reward_name}')
    axes[1].set_title('Cumulative reward in the same time horizon')
    axes[1].set_ylabel('Cumulative reward')
    axes[1].grid(alpha=0.25)
    axes[1].legend(loc='best')

    axes[2].plot(steps, fixed_slice['cumulative_grid_import_kwh'], color='#6c757d', linewidth=2.0, label='Fixed strategy')
    for reward_name, learned_run in learned_runs.items():
        learned_slice = learned_run.trajectory.head(horizon)
        axes[2].plot(steps, learned_slice['cumulative_grid_import_kwh'], linewidth=2.0, label=f'Q-learning: {reward_name}')
    axes[2].fill_between(steps, 0.0, fixed_slice['occupant_count'], color='#f4a261', alpha=0.18, label='Occupancy')
    axes[2].set_title('Energy import in the same time horizon')
    axes[2].set_xlabel('Time step')
    axes[2].set_ylabel('Cumulative import [kWh]')
...
def save_policy_context_dashboard(fixed_run: PolicyRun, learned_runs: dict[str, PolicyRun], output_path: Path, horizon: int) -> None:
    fixed_slice = fixed_run.trajectory.head(horizon)
    steps = fixed_slice['time_step'].to_numpy()
    fig, axes = plt.subplots(4, 1, figsize=(13, 11), sharex=True)

    axes[0].plot(steps, fixed_slice['action_value'], color='#6c757d', linewidth=1.8, label='Fixed strategy')
    for reward_name, learned_run in learned_runs.items():
        learned_slice = learned_run.trajectory.head(horizon)
        axes[0].plot(steps, learned_slice['action_value'], linewidth=1.8, label=f'Q-learning: {reward_name}')
    axes[0].set_title('Action comparison in the same time horizon')
    axes[0].set_ylabel('Cooling action')
    axes[0].grid(alpha=0.25)
    axes[0].legend(loc='best')

    axes[1].plot(steps, fixed_slice['weather_predicted_1'], color='#457b9d', linewidth=2.0, label='Weather +1h')
    axes[1].plot(steps, fixed_slice['weather_predicted_2'], color='#1d3557', linewidth=1.6, label='Weather +2h')
    axes[1].plot(steps, fixed_slice['weather_predicted_3'], color='#6d597a', linewidth=1.4, label='Weather +3h')
    axes[1].set_ylabel('Forecast [C]')
    axes[1].grid(alpha=0.25)
    axes[1].legend(loc='best')

    axes[2].fill_between(steps, 0.0, fixed_slice['occupant_count'], color='#f4a261', alpha=0.35, label='Occupancy count')
    ...
    for reward_name, learned_run in learned_runs.items():
        learned_slice = learned_run.trajectory.head(horizon)
        axes[3].plot(steps, learned_slice['reward'], linewidth=1.5, alpha=0.9, label=f'Reward: {reward_name}')
    axes[3].plot(steps, fixed_slice['reward'], color='#6c757d', linewidth=1.5, alpha=0.9, label='Reward: fixed strategy')
...
def run_experiment(
    ...
    learned_runs: dict[str, PolicyRun] = {}
    training_traces: dict[str, TrainingTrace] = {}

    print('Evaluating fixed strategy first...', flush=True)
    fixed_env = make_env(schema_path, building_name, reward_classes[0], random_seed)
    ...
    fixed_run.result.reward_name = 'FixedStrategyReference'
    ...
    for reward_class in reward_classes:
        ...
        print(f'Training Q-learning with {reward_name}...', flush=True)
        ...
        learned_run.result.reward_name = reward_name
        ...
        learned_runs[reward_name] = learned_run
        training_traces[reward_name] = training_trace

        reward_slug = reward_name.replace('Reward', '').lower()
        pd.DataFrame({
    ...
    save_learning_dashboard(training_traces, output_dir / 'learning_reward_comparison.png')
    save_time_comparison_dashboard(fixed_run, learned_runs, output_dir / 'time_reward_comparison.png', comparison_horizon)
    save_policy_context_dashboard(fixed_run, learned_runs, output_dir / 'policy_context_comparison.png', comparison_horizon)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Assignment-focused Q-learning comparison against a fixed strategy using weather forecasts and room occupancy.'
    )
    ...
    parser.add_argument('--comparison-horizon', type=int, default=168, help='Number of initial time steps shown in time-based comparison charts.')
    ...
    parser.add_argument(
        ...
        help='Reward configurations to compare. Supported: energy_only weather_occupancy.',
    )
    ...
    print(f'Reward configs for Q-learning: {", ".join(args.reward_configs)}', flush=True)
    ...
    print('Fixed strategy is evaluated first and then used as the baseline for energy savings comparison.', flush=True)
    print('Energy_only shows pure energy optimization.', flush=True)
    print('Weather_occupancy is the assignment-aligned reward because it emphasizes decisions with occupancy and weather forecasts.', flush=True)
    print('Learning comparison shows the effect of different reward functions on training progress and stability.', flush=True)
    print(results.to_string(index=False))
    print('Saved files:', flush=True)
    print(f'  {args.output_dir / "summary_results.csv"}', flush=True)
    print(f'  {args.output_dir / "learning_reward_comparison.png"}', flush=True)
    print(f'  {args.output_dir / "time_reward_comparison.png"}', flush=True)
    print(f'  {args.output_dir / "policy_context_comparison.png"}', flush=True)
