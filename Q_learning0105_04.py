...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning0105_04'
...
class FixedPolicy:
    def __init__(self, dhw_action: float = 0.0, electrical_action: float = 0.0, cooling_action: float = 0.5):
        action_by_name = {
            'dhw_storage': float(dhw_action),
            'electrical_storage': float(electrical_action),
            'cooling_device': float(cooling_action),
        }
        self.actions = [action_by_name[name] for name in ACTIVE_ACTIONS]
...
class ExperimentResult:
    ...
    net_grid_balance_kwh: float
    total_net_consumption_kwh: float
    discomfort_proportion: float
    all_rewards: float
    cost_total: float | None = None
    carbon_emissions_total: float | None = None
    daily_peak_average: float | None = None
    ramping_average: float | None = None
    zero_net_energy: float | None = None
    cost_vs_fixed_pct: float | None = None
    emissions_vs_fixed_pct: float | None = None
    peak_vs_fixed_pct: float | None = None
    savings_vs_fixed_pct: float | None = None
    reward_vs_fixed_pct: float | None = None
...
def eval_agent(env: CityLearnEnv, act_fn) -> PolicyRun:
    ...
    return PolicyRun(
        result=ExperimentResult(
            ...
            net_grid_balance_kwh=float(np.sum(net_grid_balance)),
            total_net_consumption_kwh=float(np.sum(aggregate_net)),
            discomfort_proportion=discomfort,
            all_rewards=all_rewards,
            cost_total=float(district_metric_map['cost_total']) if 'cost_total' in district_metric_map and pd.notna(district_metric_map['cost_total']) else None,
            carbon_emissions_total=float(district_metric_map['carbon_emissions_total']) if 'carbon_emissions_total' in district_metric_map and pd.notna(district_metric_map['carbon_emissions_total']) else None,
            daily_peak_average=float(district_metric_map['daily_peak_average']) if 'daily_peak_average' in district_metric_map and pd.notna(district_metric_map['daily_peak_average']) else None,
            ramping_average=float(district_metric_map['ramping_average']) if 'ramping_average' in district_metric_map and pd.notna(district_metric_map['ramping_average']) else None,
            zero_net_energy=float(district_metric_map['zero_net_energy']) if 'zero_net_energy' in district_metric_map and pd.notna(district_metric_map['zero_net_energy']) else None,
        ),
    ...
    return pd.DataFrame([
        {
            ...
            'net_grid_balance_kwh': round(result.net_grid_balance_kwh, 3),
            ...
            'cost_total': None if result.cost_total is None else round(result.cost_total, 4),
            'carbon_emissions_total': None if result.carbon_emissions_total is None else round(result.carbon_emissions_total, 4),
            'daily_peak_average': None if result.daily_peak_average is None else round(result.daily_peak_average, 4),
            'ramping_average': None if result.ramping_average is None else round(result.ramping_average, 4),
            'zero_net_energy': None if result.zero_net_energy is None else round(result.zero_net_energy, 4),
            'cost_vs_fixed_pct': None if result.cost_vs_fixed_pct is None else round(result.cost_vs_fixed_pct, 3),
            'emissions_vs_fixed_pct': None if result.emissions_vs_fixed_pct is None else round(result.emissions_vs_fixed_pct, 3),
            'peak_vs_fixed_pct': None if result.peak_vs_fixed_pct is None else round(result.peak_vs_fixed_pct, 3),
            'savings_vs_fixed_pct': None if result.savings_vs_fixed_pct is None else round(result.savings_vs_fixed_pct, 3),
            'reward_vs_fixed_pct': None if result.reward_vs_fixed_pct is None else round(result.reward_vs_fixed_pct, 3),
...
def save_policy_comparison_figure(results_frame: pd.DataFrame, policy_runs: list[PolicyRun], output_path: Path) -> None:
    ...
    axes[0, 1].bar(x_axis, results_frame['cost_total'], color=colors)
    axes[0, 1].set_title('Cost index (CityLearn KPI)')
    axes[0, 1].set_ylabel('ratio')
    axes[0, 1].set_xticks(x_axis, labels, rotation=35, ha='right', fontsize=8)
    axes[0, 1].grid(axis='y', alpha=0.25)

    axes[0, 2].bar(x_axis, results_frame['carbon_emissions_total'], color=colors)
    axes[0, 2].set_title('Carbon emissions index')
    axes[0, 2].set_ylabel('ratio')
    axes[0, 2].set_xticks(x_axis, labels, rotation=35, ha='right', fontsize=8)
    axes[0, 2].grid(axis='y', alpha=0.25)

    axes[1, 0].bar(x_axis, results_frame['daily_peak_average'], color=colors)
    axes[1, 0].set_title('Daily peak average')
    axes[1, 0].set_ylabel('ratio')
    axes[1, 0].set_xticks(x_axis, labels, rotation=35, ha='right', fontsize=8)
    axes[1, 0].grid(axis='y', alpha=0.25)

    axes[1, 1].bar(x_axis, results_frame['ramping_average'], color=colors)
    axes[1, 1].set_title('Ramping average')
    axes[1, 1].set_ylabel('ratio')
    axes[1, 1].set_xticks(x_axis, labels, rotation=35, ha='right', fontsize=8)
    ...
    axes[1, 2].set_title('Average grid import over 14-day profile')
    axes[1, 2].set_xlabel('Hour in 14-day cycle')
    axes[1, 2].set_ylabel('kWh')
    axes[1, 2].set_xticks(range(0, profile_hours + 1, 24))
    axes[1, 2].grid(alpha=0.25)
    axes[1, 2].legend(loc='best', fontsize=7, ncol=2)

    fig.suptitle(f'1 budova: Fixed vs {len(labels) - 1} reward variantov Q-learningu')
    fig.tight_layout()
...
def save_time_and_learning_comparison(fixed_run: PolicyRun, learned_run: PolicyRun, training_trace: TrainingTrace, output_path: Path, horizon: int) -> None:
    ...
    axes[0, 1].plot(learned_slice['time_step'], learned_slice['all_rewards'], lw=1.9, color='#1d3557', label='Q-learning')
    ...
    fig.suptitle('2 akcie: DHW zasobnik + bateria')
    fig.tight_layout()
    fig.savefig(output_path, dpi=160)
    plt.close(fig)
...
def run_experiment(
    ...
    fixed_run.result.cost_vs_fixed_pct = 0.0
    fixed_run.result.emissions_vs_fixed_pct = 0.0
    fixed_run.result.peak_vs_fixed_pct = 0.0
    ...
    for key, display_name, reward_cls in REWARD_CONFIGS:
        ...
        if fixed_run.result.total_grid_import_kwh > 0.0:
            # This compares policies by energy imported from the grid, not by reward.
        ...
        if fixed_run.result.cost_total is not None and fixed_run.result.cost_total > 0.0 and run.result.cost_total is not None:
            run.result.cost_vs_fixed_pct = 100.0 * (run.result.cost_total - fixed_run.result.cost_total) / fixed_run.result.cost_total
        if fixed_run.result.carbon_emissions_total is not None and fixed_run.result.carbon_emissions_total > 0.0 and run.result.carbon_emissions_total is not None:
            run.result.emissions_vs_fixed_pct = 100.0 * (run.result.carbon_emissions_total - fixed_run.result.carbon_emissions_total) / fixed_run.result.carbon_emissions_total
        if fixed_run.result.daily_peak_average is not None and fixed_run.result.daily_peak_average > 0.0 and run.result.daily_peak_average is not None:
            run.result.peak_vs_fixed_pct = 100.0 * (run.result.daily_peak_average - fixed_run.result.daily_peak_average) / fixed_run.result.daily_peak_average
        if abs(fixed_run.result.all_rewards) > 0.0:
            run.result.reward_vs_fixed_pct = 100.0 * (run.result.all_rewards - fixed_run.result.all_rewards) / abs(fixed_run.result.all_rewards)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Q-learning experiment pre 1 budovu s pricing a storage reward variantmi.')
