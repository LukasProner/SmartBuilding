...
EVAL_EPISODES = 30
FIXED_BASELINE_ACTION = 1  # 0=off, 1=low, 2=high
...
def build_eval_start_indices(n_steps: int, horizon: int, n_episodes: int) -> np.ndarray:
    max_start = max(0, n_steps - horizon)
    if n_episodes <= 1 or max_start == 0:
        return np.array([0], dtype=np.int64)
    return np.linspace(0, max_start, num=n_episodes, dtype=np.int64)
...
class BuildingEnv:
    ...
    def reset(self, start_index=None, random_start=True):
        if start_index is not None:
            self.t = int(start_index)
        elif random_start:
            max_start = max(0, self.n_steps - EPISODE_LENGTH)
    ...
    def step(self, action: int):
        ...
        net_grid_energy = max(0.0, base_load + hvac_energy - solar_generation)

        # Penalize unmet comfort when HVAC is off while demand is high.
        comfort_penalty = 0.0
        if action == 0 and hvac_demand > 0.5:
            comfort_penalty += 0.3 * hvac_demand

        reward = -net_grid_energy - comfort_penalty
...
def train(env: BuildingEnv, agent: QLearningAgent, edges):
    ...
    for _ in tqdm(range(TRAIN_EPISODES), desc='Training Q-learning'):
        ...
        for _step in range(EPISODE_LENGTH):
            ...
            ep_reward += reward
            ep_energy += info['net_grid_energy']
...
def evaluate(env: BuildingEnv, policy_fn, edges, start_indices=None):
    ep_energies = []
    ep_rewards = []
    ep_penalties = []
    ep_objectives = []
    ...
    for start_idx in start_indices:
        state = env.reset(start_index=int(start_idx), random_start=False)
        ep_energy = 0.0
        ep_reward = 0.0
        ep_penalty = 0.0
        ...
        for _step in range(EPISODE_LENGTH):
            ...
            ep_energy += info['net_grid_energy']
            ep_reward += reward
            ep_penalty += info['comfort_penalty']
        ...
        ep_objectives.append(ep_energy + ep_penalty)

    return {
        'avg_energy': float(np.mean(ep_energies)),
        'avg_reward': float(np.mean(ep_rewards)),
        'avg_penalty': float(np.mean(ep_penalties)),
        'avg_objective': float(np.mean(ep_objectives)),
        'episode_energies': ep_energies,
    }


def rbc_policy(_state_idx, state_info):
    occupancy = state_info['occupancy']
    temp_mean = state_info['forecast_temp_mean']
...
def main():
    ...
    q_policy = lambda s, _state: agent.act(s, training=False)
    eval_starts = build_eval_start_indices(len(df), EPISODE_LENGTH, EVAL_EPISODES)
    q_res = evaluate(env, q_policy, edges, start_indices=eval_starts)
    print(f'    Q-learning avg energy:    {q_res["avg_energy"]:.4f}')
    print(f'    Q-learning avg penalty:   {q_res["avg_penalty"]:.4f}')
    print(f'    Q-learning avg objective: {q_res["avg_objective"]:.4f}')

    print('\n[5] Evaluating fixed baselines...')
    baseline_results = {}
    for a_id, a_name in ACTIONS.items():
        fixed = lambda _s, _state, aa=a_id: aa
        res = evaluate(env, fixed, edges, start_indices=eval_starts)
        baseline_results[a_name] = res
        print(f'    {a_name:10s} avg objective: {res["avg_objective"]:.4f} (energy={res["avg_energy"]:.4f}, penalty={res["avg_penalty"]:.4f})')

    rbc_res = evaluate(env, rbc_policy, edges, start_indices=eval_starts)
    print(f'    rbc        avg objective: {rbc_res["avg_objective"]:.4f} (energy={rbc_res["avg_energy"]:.4f}, penalty={rbc_res["avg_penalty"]:.4f})')

    print('\n[6] Objective savings vs reference baselines')
    best_name, best_res = min(baseline_results.items(), key=lambda kv: kv[1]['avg_objective'])
    reference_name = ACTIONS[FIXED_BASELINE_ACTION]
    reference_res = baseline_results[reference_name]
    objective_savings = best_res['avg_objective'] - q_res['avg_objective']
    objective_savings_pct = 100.0 * objective_savings / best_res['avg_objective'] if best_res['avg_objective'] > 0 else 0.0
    reference_savings = reference_res['avg_objective'] - q_res['avg_objective']
    reference_savings_pct = 100.0 * reference_savings / reference_res['avg_objective'] if reference_res['avg_objective'] > 0 else 0.0
    rbc_savings = rbc_res['avg_objective'] - q_res['avg_objective']
    rbc_savings_pct = 100.0 * rbc_savings / rbc_res['avg_objective'] if rbc_res['avg_objective'] > 0 else 0.0

    print('-' * 72)
    print(f'Q-learning objective:         {q_res["avg_objective"]:.4f}')
    print(f'Best fixed objective ({best_name}): {best_res["avg_objective"]:.4f}')
    print(f'Objective savings vs best fixed:    {objective_savings:.4f} ({objective_savings_pct:.2f}%)')
    print(f'Objective savings vs fixed {reference_name}: {reference_savings:.4f} ({reference_savings_pct:.2f}%)')
    print(f'Objective savings vs RBC:           {rbc_savings:.4f} ({rbc_savings_pct:.2f}%)')
    print('-' * 72)
