...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning1504_06'
ACTIVE_OBSERVATIONS = [
    'occupant_count',
...
class OccupancyWeightedReward(RewardFunction):
    def __init__(self, env_metadata, occupancy_weight: float = 1.5, **kwargs):
    ...
    def calculate(self, observations: list[dict]) -> list[float]:
        ...
        for observation in observations:
            ...
            reward_list.append(-(grid_import * occupancy_factor))

        return [sum(reward_list)] if self.central_agent else reward_list


class WeatherWeightedReward(RewardFunction):
    def __init__(self, env_metadata, hot_weather_weight: float = 0.07, **kwargs):
        super().__init__(env_metadata, **kwargs)
    ...
    def calculate(self, observations: list[dict]) -> list[float]:
        ...
        for observation in observations:
            ...
            reward_list.append(-(grid_import * weather_factor))
...
def save_single_comparison_figure(results_frame: pd.DataFrame, output_path: Path) -> None:
    labels = results_frame['reward'].tolist()
    ...
    fig.suptitle('Reward-function comparison against fixed strategy', fontsize=13)
    fig.tight_layout()
...
def save_time_and_learning_comparison(
    fixed_runs: dict[str, PolicyRun],
    learned_runs: dict[str, PolicyRun],
    training_traces: dict[str, TrainingTrace],
    output_path: Path,
    horizon: int,
) -> None:
    reward_names = list(learned_runs.keys())
    color_map = {
        'EnergyOnlyReward': '#1d3557',
        'OccupancyWeightedReward': '#2a9d8f',
        'WeatherWeightedReward': '#e76f51',
        'WeatherOccupancyReward': '#6d597a',
    }

    fig, axes = plt.subplots(2, 2, figsize=(14, 9))

    for reward_name in reward_names:
        color = color_map.get(reward_name, None)
        fixed_slice = fixed_runs[reward_name].trajectory.head(horizon)
        learned_slice = learned_runs[reward_name].trajectory.head(horizon)
        axes[0, 0].plot(fixed_slice['time_step'], fixed_slice['reward'], linestyle='--', linewidth=1.5, color=color, alpha=0.7, label=f'Fixed: {reward_name}')
        axes[0, 0].plot(learned_slice['time_step'], learned_slice['reward'], linewidth=1.8, color=color, label=f'Q-learning: {reward_name}')
        axes[0, 1].plot(fixed_slice['time_step'], fixed_slice['cumulative_reward'], linestyle='--', linewidth=1.6, color=color, alpha=0.7)
        axes[0, 1].plot(learned_slice['time_step'], learned_slice['cumulative_reward'], linewidth=2.0, color=color)
        axes[1, 1].plot(fixed_slice['time_step'], fixed_slice['cumulative_grid_import_kwh'], linestyle='--', linewidth=1.6, color=color, alpha=0.7)
        axes[1, 1].plot(learned_slice['time_step'], learned_slice['cumulative_grid_import_kwh'], linewidth=2.0, color=color)
    ...
    for reward_name, trace in training_traces.items():
        ...
        axes[1, 0].plot(episodes, rolling, linewidth=2.0, color=color, label=reward_name)
    ...
    axes[1, 0].set_title('Learning progress by reward function')
    ...
    fig.suptitle('Reward functions: time behavior and learning comparison', fontsize=13)
    fig.tight_layout()
    fig.savefig(output_path, dpi=160)
    plt.close(fig)
...
def parse_reward_names(reward_names: Sequence[str]) -> list[Type[RewardFunction]]:
    reward_map: dict[str, Type[RewardFunction]] = {
        ...
        'occupancy_weighted': OccupancyWeightedReward,
        'weather_weighted': WeatherWeightedReward,
...
def run_experiment(
    ...
    for reward_class in reward_classes:
        ...
        print(f'Evaluating fixed strategy for {reward_name}...', flush=True)
        fixed_env = make_env(schema_path, building_name, reward_class, random_seed)
        ...
        fixed_run.result.reward_name = reward_name
        fixed_run.result.savings_vs_fixed_pct = 0.0
        results.append(fixed_run.result)
        fixed_runs[reward_name] = fixed_run
        ...
        if fixed_runs[reward_name].result.total_grid_import_kwh > 0.0:
            learned_run.result.savings_vs_fixed_pct = 100.0 * (
                fixed_runs[reward_name].result.total_grid_import_kwh - learned_run.result.total_grid_import_kwh
            ) / fixed_runs[reward_name].result.total_grid_import_kwh
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Q-learning comparison with several reward functions and one summary graph against a fixed strategy.'
    )
    ...
    parser.add_argument('--comparison-horizon', type=int, default=168, help='Number of initial time steps shown in the time comparison figure.')
    ...
    parser.add_argument(
        '--reward-configs',
        nargs='+',
        default=['energy_only', 'occupancy_weighted', 'weather_weighted', 'weather_occupancy'],
        help='Reward configurations to compare. Supported: energy_only occupancy_weighted weather_weighted weather_occupancy.',
    )
    ...
    print('Reward functions in this version:', flush=True)
    print('  energy_only = pure energy minimization', flush=True)
    print('  occupancy_weighted = stronger penalty when occupancy is high', flush=True)
    print('  weather_weighted = stronger penalty when forecast temperature is high', flush=True)
    print('  weather_occupancy = combined weather + occupancy weighting', flush=True)
