...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning1704_01_withSolar'

# ── Observations ─────────────────────────────────────────────────────────────
# ROZŠÍRENÉ OBSERVÁCIE: pridané difúzne slnečné žiarenie s predpovedami (1-3 hodiny)
# Difúzne žiarenie (diffuse) je čiastka "rozptýleného" svetla, ktorá je viditeľná
# z každého uhla — vhodné pre fotovoltiku aj iné aplikácie.
ACTIVE_OBSERVATIONS = [
    'outdoor_dry_bulb_temperature_predicted_1',    # teplota za 1 hod
    'outdoor_dry_bulb_temperature_predicted_2',    # teplota za 2 hod
    'outdoor_dry_bulb_temperature_predicted_3',    # teplota za 3 hod
    'diffuse_solar_irradiance_predicted_1',        # difúzne žiarenie za 1 hod [W/m²]
    'diffuse_solar_irradiance_predicted_2',        # difúzne žiarenie za 2 hod [W/m²]
    'electrical_storage_soc',                      # stav nabití batérie (0–1)
    'occupant_count',                              # počet ľudí v budove
]

# Bin counts: hrubšia diskretizácia teplot, solárne žiarenie má menšie rozsahy
# Difúzne žiarenie sa pohybuje od 0 do ~300 W/m², je vhodné rozdeliť na 4 koše
OBSERVATION_BIN_SIZES = {
    'outdoor_dry_bulb_temperature_predicted_1': 5,   # najdôležitejšia teplota
    'outdoor_dry_bulb_temperature_predicted_2': 4,   # menej presná
    'outdoor_dry_bulb_temperature_predicted_3': 4,   # najmenej presná
    'diffuse_solar_irradiance_predicted_1': 4,       # žiarenie za 1 hod
    'diffuse_solar_irradiance_predicted_2': 4,       # žiarenie za 2 hod
    'electrical_storage_soc': 3,                     # nízky / stredný / vysoký
    'occupant_count': 4,                             # 0, 1-2, 3-4, 5+
}
# Veľkosť stavového priestoru: 5×4×4×4×4×3×4 = 7680 stavov (2.67× viac ako bez solárneho žiarenia)

# ── Akcie ────────────────────────────────────────────────────────────────────
# Poradie: [dhw_storage, electrical_storage, cooling_device]
ACTIVE_ACTIONS = ['dhw_storage', 'electrical_storage', 'cooling_device']

# Počet binov pre každú akciu
# Zásobníky majú 3 biny: -1 (vybíjaj) / 0 (nič) / +1 (nabíjaj)
...
class WeatherOccupancyReward(RewardFunction):
    ...
    Pozn.: Nepriamo učí agenta, aby využíval slnečné žiarenie (cez energetiku),
    avšak observácie sú teraz rozšírené na solar irradiance, čo dáva agentovi
    explicitný signál pre lepšie plánovanie nabíjania batérie.
    """
...
# ── Diskretizácia akčného priestoru ──────────────────────────────────────────
class MultiActionDiscretizer:
    """Vytvára Cartesian product akcií."""
...
def save_single_comparison_figure(results_frame: pd.DataFrame, output_path: Path) -> None:
    ...
    fig.suptitle('3 akcie s SOLÁRNYM ŽIARENÍM: fixná vs Q-learning', fontsize=13)
    fig.tight_layout()
...
def save_time_and_learning_comparison(
    ...
    fig.suptitle('3 akcie s SOLÁRNYM ŽIARENÍM: DHW + batéria + chladenie', fontsize=13)
    fig.tight_layout()
    fig.savefig(output_path, dpi=160)
    plt.close(fig)
...
def run_experiment(
    ...
    # ── Q-learning s solárnym žiarením ──
    print('Training Q-learning with 3 actions (+ SOLAR IRRADIANCE observations)...', flush=True)
    ...
    learned_run.result.policy = 'Q-learning (3 actions + Solar)'
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Q-learning so všetkými 3 akciami + SOLÁRNYM ŽIARENÍM (diffuse_solar_irradiance_predicted) na jednej budove.'
    )
    parser.add_argument('--schema', type=Path, default=DEFAULT_SCHEMA)
    parser.add_argument('--building', type=str, default=DEFAULT_BUILDING)
    parser.add_argument('--episodes', type=int, default=800,
                        help='Počet trénovacích epizód. Odporúčané min. 800 kvôli väčšiemu stavovému priestoru (7680 stavov).')
    ...
    print(f'Observations (7): {", ".join(ACTIVE_OBSERVATIONS)}', flush=True)
    print(f'  → Includes SOLAR IRRADIANCE predictions (1-2 hours ahead)', flush=True)
    ...
    print(f'State space: 5×4×4×4×4×3×4 = 7680 stavov (2.67× bigger than without solar)', flush=True)
