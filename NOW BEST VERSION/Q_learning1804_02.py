...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning1804_02'
...
# Veľkosť stavového priestoru: 3×3×3×3×3×3×4 = 2916 stavov
...
def save_policy_comparison_figure(
    ...
    fixed_run: PolicyRun,
    learned_weather_run: PolicyRun,
    learned_energy_run: PolicyRun,
    output_path: Path,
) -> None:
    labels = results_frame['policy'].tolist()
    x = np.arange(len(labels))
    colors = ['#9aa0a6', '#1d3557', '#2a9d8f']
    ...
    fixed_hourly = fixed_run.trajectory.groupby(fixed_run.trajectory['time_step'] % 24)['grid_import_kwh'].mean()
    weather_hourly = learned_weather_run.trajectory.groupby(
        learned_weather_run.trajectory['time_step'] % 24
    )['grid_import_kwh'].mean()
    energy_hourly = learned_energy_run.trajectory.groupby(
        learned_energy_run.trajectory['time_step'] % 24
    )['grid_import_kwh'].mean()
    hours = np.arange(24)
    axes[2].plot(hours, fixed_hourly.reindex(hours, fill_value=np.nan).to_numpy(), linestyle='--', linewidth=2.0, color='#9aa0a6', label='Fixed')
    axes[2].plot(hours, weather_hourly.reindex(hours, fill_value=np.nan).to_numpy(), linewidth=2.0, color='#1d3557', label='Q-learning Weather')
    axes[2].plot(hours, energy_hourly.reindex(hours, fill_value=np.nan).to_numpy(), linewidth=2.0, color='#2a9d8f', label='Q-learning Energy')
    axes[2].set_title('Hourly average grid import')
    ...
    fig.suptitle('3 akcie: Fixed vs 2 reward varianty Q-learningu', fontsize=13)
    fig.tight_layout()
...
def run_experiment(
    ...
    weather_env = make_env(schema_path, building_name, random_seed, reward_function=WeatherOccupancyReward)
    ...
    energy_env = make_env(schema_path, building_name, random_seed, reward_function=GridImportOnlyReward)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Q-learning (compact-state): teplota/solar ako level+trend, porovnanie 2 reward funkcií voči fixnej stratégii.'
    )
    ...
    print('Reward functions: WeatherOccupancyReward, GridImportOnlyReward', flush=True)
    ...
    for fname in [
        'summary_results.csv',
        'learning_trace_weather.csv', 'learning_trace_energy.csv',
        'q_table_weather.npy', 'q_table_energy.npy',
        'trajectory_fixed_strategy.csv',
        'trajectory_q_learning_weather.csv', 'trajectory_q_learning_energy.csv',
