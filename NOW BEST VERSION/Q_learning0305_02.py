...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning0305_02'
DEFAULT_EPISODES = 50
...
class TimeOfUseReward(RewardFunction):
    def __init__(self, env_metadata, peak_multiplier: float = 4.0, night_discount: float = 0.4, **kwargs):
        super().__init__(env_metadata, **kwargs)
...
class SmartTouReward(RewardFunction):
...
REWARD_CONFIGS: list[tuple[str, str, type[RewardFunction]]] = [
    ...
    ('nightpre', 'NightPrecharge reward', SmartTouReward),
]
...
def get_reward_configs(selected_reward_keys: list[str] | None = None) -> list[tuple[str, str, type[RewardFunction]]]:
    if not selected_reward_keys:
        return list(REWARD_CONFIGS)

    requested = {str(key).strip().lower() for key in selected_reward_keys}
    matched = [config for config in REWARD_CONFIGS if config[0].lower() in requested]
    missing = sorted(requested - {config[0].lower() for config in matched})
    if missing:
        available = ', '.join(key for key, _, _ in REWARD_CONFIGS)
        raise ValueError(f'Unknown reward key(s): {", ".join(missing)}. Available reward keys: {available}.')

    return matched


def merge_summary_results(output_dir: Path, latest_results: pd.DataFrame) -> pd.DataFrame:
    summary_path = output_dir / 'summary_results.csv'
    if summary_path.exists():
        existing = pd.read_csv(summary_path)
        combined = pd.concat([existing, latest_results], ignore_index=True)
        combined = combined.drop_duplicates(subset=['policy'], keep='last')
    else:
        combined = latest_results.copy()

    fixed_mask = combined['policy'].astype(str).str.startswith('Fixed(')
    fixed_rows = combined[fixed_mask].drop_duplicates(subset=['policy'], keep='last')
    learned_rows = combined[~fixed_mask].sort_values(by='policy').drop_duplicates(subset=['policy'], keep='last')
    merged = pd.concat([fixed_rows, learned_rows], ignore_index=True)
...
def run_experiment(
    ...
    selected_reward_keys: list[str] | None = None,
) -> pd.DataFrame:
    ...
    reward_configs = get_reward_configs(selected_reward_keys)
    ...
    latest_results_frame = build_results_frame(results).reset_index(drop=True)
    results_frame = merge_summary_results(output_dir, latest_results_frame)
...
def main() -> None:
    parser = argparse.ArgumentParser(description='Run Q-learning experiments for CityLearn reward functions.')
    parser.add_argument(
        '--reward',
        dest='reward_keys',
        action='append',
        help='Reward key to run. Repeat the flag to run multiple rewards. Defaults to all configured rewards.',
    )
    parser.add_argument('--episodes', type=int, default=DEFAULT_EPISODES)
    parser.add_argument('--output-dir', type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument('--list-rewards', action='store_true', help='Print configured reward keys and exit.')
    args = parser.parse_args()

    if args.list_rewards:
        for key, display_name, _reward_cls in REWARD_CONFIGS:
    ...
    results = run_experiment(
        ...
        selected_reward_keys=args.reward_keys,
    )
