...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning0505_02'
    ...
    for episode in range(episodes):
        ...
        if current_episode == 1 or current_episode % progress_every == 0 or current_episode == episodes:
            ...
            print(
                ...
                f'| rolling10={rolling_reward:.2f} | epsilon={agent.epsilon:.3f} | elapsed={elapsed:.1f}s',
                flush=True,
            )
...
def save_policy_comparison_figure(results_frame: pd.DataFrame, policy_runs: list[PolicyRun], output_path: Path) -> None:
    ...
    fig.suptitle(f'3 budovy: Fixed vs {len(labels) - 1} reward variantov Q-learning')
    fig.tight_layout()
...
def run_experiment(
    ...
    print('Running fixed strategy...', flush=True)
...
def main() -> None:
    ...
    parser.add_argument('--comparison-horizon', type=int, default=DEFAULT_COMPARISON_HORIZON)
