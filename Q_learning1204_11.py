#!/usr/bin/env python
"""
Q_learning1204_11.py
====================
Tabulárny Q-learning pre CityLearn Challenge 2022 Phase 1.

Hlavná zmena oproti Q_learning1204_10.py
-----------------------------------------
Stav agenta teraz obsahuje skutočné predpovede počasia a obsadenosti:

    (hodina, solar_bin, temp_bin, load_bin, soc_bin)

  • solar_bin  – predpoveď difúzneho + priameho solárneho žiarenia (W/m²)
                 → reprezentuje slnečné podmienky (cloudy/partly/clear)
  • temp_bin   – predpoveď vonkajšej teploty (°C)
                 → ovplyvňuje potrebu chladenia/kúrenia, teda load budovy
  • load_bin   – predpoveď non_shiftable_load (kWh)
                 → proxy pre obsadenosť budovy (viac ľudí ≈ vyšší load)
  • soc_bin    – stav nabitia batérie (pevné fyzikálne hranice)
  • hodina     – čas dňa (24 hodnôt)

Počet diskrétnych stavov/budovu:
    24 × 5 × 4 × 4 × 6 = 11 520  (zvládnuteľné pre tabulárny Q-learning)

Všetky ostatné vylepšenia z verzie 10 sú zachované:
  – pevné SOC biny [0.10 … 0.90]
  – maskovanie akcií pri prázdnej/plnej batérii
  – warmup so striedavým nabíjaním/vybíjaním
  – safe_pct() – N/A pri zápornom baseline nákladoch
"""
...
RANDOM_SEED   = 42
OUTPUT_PREFIX = "q_learning1204_11"
...
WARMUP_EPISODES = 8   # pre výpočet solar/temp/load binov
...
# Pevné SOC biny – fyzikálne zmysluplné
...
TEMP_FALLBACK  = np.array([10.0, 20.0,  30.0],          dtype=np.float32)   # °C
...
def quantile_bins(values: List[float], n: int, fallback: np.ndarray) -> np.ndarray:
    """Vytvorí n hraničných bodov z kvantilov dát. Vracia fallback ak málo vzoriek."""
...
class BinSet:
    """
    Stavové biny pre (hodina, solar, teplota, load, SOC).

    solar → quantile biny z warmup dát (n=4 edges → 5 intervalov)
    temp  → quantile biny z warmup dát (n=3 edges → 4 intervaly)
    load  → quantile biny z warmup dát (n=3 edges → 4 intervaly)
    SOC   → vždy pevné fyzikálne hranice
    """

    def __init__(self, env: CityLearnEnv) -> None:
        idx = get_obs_index(env)

        # ── Kľúče pozorovateľných veličín ──────────────────────────────────
        ...
        # Predpoveď vonkajšej teploty = počasie
        self.k_temp = first_key(idx, [
            "outdoor_dry_bulb_temperature_predicted_6h",
            "outdoor_dry_bulb_temperature_predicted_1",
            "outdoor_dry_bulb_temperature",
        ])
        # Non-shiftable load = proxy pre obsadenosť budovy
    ...
    def fit(self, env: CityLearnEnv) -> None:
        """
        Warmup epizódy so striedavým nabíjaním / vybíjaním.
        Zbiera solar, temp a load vzorky → quantile biny.
        """
        solar_v: List[float] = []
        temp_v:  List[float] = []
        load_v:  List[float] = []
        ...
        for ep_i in range(WARMUP_EPISODES):
            act_val = 1.0 if ep_i % 2 == 0 else -1.0   # striedavé nabíjanie/vybíjanie
            env.reset()
            for _ in range(EPISODE_TIME_STEPS):
                ...
                for obs in next_obs_list:
                    ...
                    if self.i_temp is not None:
                        temp_v.append(float(obs[self.i_temp]))
        ...
        self._temp_samples  = temp_v
        self._load_samples  = load_v

        # n=4 → 4 hraničné body → 5 intervalov pre solar
        # n=3 → 3 hraničné body → 4 intervaly pre temp a load
        self.solar_bins = quantile_bins(solar_v, 4, SOLAR_FALLBACK)
        self.temp_bins  = quantile_bins(temp_v,  3, TEMP_FALLBACK)
        self.load_bins  = quantile_bins(load_v,  3, LOAD_FALLBACK)

    # ------------------------------------------------------------------
    def state_space_size(self) -> int:
        n_solar = len(self.solar_bins) + 1
        n_temp  = len(self.temp_bins)  + 1
        n_load  = len(self.load_bins)  + 1
        n_soc   = len(self.soc_bins)   + 1
        return 24 * n_solar * n_temp * n_load * n_soc
    ...
    def print_summary(self) -> None:
        ...
        rows = [
            ...
            ("teplota",     self.k_temp    or NF, self._temp_samples,    self.temp_bins),
            ("load (obs.)", self.k_load    or NF, self._load_samples,    self.load_bins),
        ...
        print(f"    {sep}")
        print(f"    Celkový stavový priestor (24 × solar × temp × load × SOC) = {n_states:,} stavov/budovu\n")
...
# 5-tica: (hodina, solar_bin, temp_bin, load_bin, soc_bin)
...
def encode_state(obs: List[float], bins: BinSet, soc: float) -> StateT:
    ...
    Args:
        obs:  vektor pozorovania danej budovy
        bins: BinSet s natrénovanými hranicami
        soc:  aktuálny SOC batérie tejto budovy (0.0 – 1.0)
...
def get_socs(env: CityLearnEnv, obs_list: List[List[float]], bins: BinSet) -> List[float]:
    """SOC každej budovy z observations."""
    return [
...
def plot_training(
    ...
    fig.suptitle("Q_learning1204_11 – Tréningové krivky  (stav: hodina+solar+teplota+load+SOC)",
...
def plot_eval_comparison(baseline: EvalMetrics, q_res: EvalMetrics) -> None:
    ...
    fig.suptitle(
        "Porovnanie per-epizóda: Fixná stratégia vs Q-learning (1204_11)\n"
        "stav: (hodina, solar, teplota, load-obsadenosť, SOC)",
        fontsize=12, fontweight="bold"
    )
    w  = 0.38
    x  = np.arange(n)
...
def main() -> None:
    ...
    print(SEP)
    print("  Q_learning1204_11 – CityLearn tabular Q-learning")
    print("  Stav: (hodina, solar, TEPLOTA, LOAD/obsadenosť, SOC)")
    print(SEP)
    ...
    print(f"  akcie              : {ACTION_LEVELS.tolist()}  (maskované pri SOC < {SOC_EMPTY_THRESH} / > {SOC_FULL_THRESH})")
    ...
    print(f"       dostupné pozorovania (1. budova): {train_env.observation_names[0]}")

    # ── [2] Warmup – biny ───────────────────────────────────────────────────
    print(f"\n[2/5] Warmup – výpočet solar/temp/load binov ({WARMUP_EPISODES} ep.) …")
    ...
    # Priemery len z platných riadkov
