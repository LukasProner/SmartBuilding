...
def run_policy(agent, env: CityLearnEnv, deterministic: bool = True) -> tuple[ExperimentResult, pd.DataFrame]:
    observations, _ = env.reset()
    agent.reset()
    terminated = False
    cumulative_reward = 0.0
...
def run_experiment(
    ...
    baseline_agent = FixedCoolingPolicy(action_value=baseline_action)
    baseline_result, _ = run_policy(baseline_agent, baseline_env, deterministic=True)
    baseline_result.policy = f'FixedCooling({baseline_action:.2f})'
    baseline_result.reward_name = 'ReferencePolicy'
    baseline_result.savings_vs_fixed_pct = 0.0
    ...
    for reward_class in reward_classes:
        ...
        training_start = time.perf_counter()
        episode_rewards = train_q_learning(agent, training_env, episodes=episodes, progress_every=1)
        training_seconds = time.perf_counter() - training_start
        evaluation_result, _ = run_policy(agent, training_env, deterministic=True)
        evaluation_result.policy = 'OwnTabularQLearning'
        evaluation_result.training_seconds = training_seconds
        evaluation_result.stability_episode = estimate_stability_episode(episode_rewards)
        evaluation_result.last_10_episode_reward_mean = float(np.mean(episode_rewards[-10:])) if episode_rewards else None

        if baseline_result.total_grid_import_kwh > 0.0:
            evaluation_result.savings_vs_fixed_pct = 100.0 * (
                baseline_result.total_grid_import_kwh - evaluation_result.total_grid_import_kwh
            ) / baseline_result.total_grid_import_kwh
        else:
            evaluation_result.savings_vs_fixed_pct = None

        results.append(evaluation_result)

    results_frame = build_results_frame(results)
    return results_frame.sort_values(['policy', 'reward']).reset_index(drop=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Tabular Q-learning experiment for weather- and occupancy-aware cooling control in CityLearn.'
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.70,
        help='Fixed cooling action used in the reference strategy.',
    )
