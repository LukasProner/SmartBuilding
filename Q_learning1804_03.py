...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning1804_03'
...
def save_policy_comparison_figure(
    ...
    colors = ['#9aa0a6', '#1d3557', '#2a9d8f', '#e76f51']
    ...
    axes[2].set_title('Average grid import over 14-day profile')
    axes[2].set_xlabel('Hour in 14-day cycle')
    axes[2].set_ylabel('kWh')
    axes[2].set_xticks(range(0, profile_hours + 1, 24))
    axes[2].grid(alpha=0.25)
    axes[2].legend(loc='best', fontsize=8)

    fig.suptitle('3 budovy: Fixed vs 3 reward varianty Q-learningu', fontsize=13)
    fig.tight_layout()
...
def run_experiment(
    ...
    save_policy_comparison_figure(
        results_frame,
        [fixed_run, learned_weather_run, learned_energy_run, learned_price_run],
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Q-learning (compact-state, 3 buildings): teplota/solar/pricing ako level+trend, porovnanie 3 reward funkcií voči fixnej stratégii.'
    )
    ...
    print('Reward functions: WeatherOccupancyReward, GridImportOnlyReward, PricingAwareReward', flush=True)
    ...
    for fname in [
        'summary_results.csv',
        'learning_trace_weather.csv', 'learning_trace_energy.csv', 'learning_trace_pricing.csv',
        'q_table_weather.npy', 'q_table_energy.npy', 'q_table_pricing.npy',
        'trajectory_fixed_strategy.csv',
        'trajectory_q_learning_weather.csv', 'trajectory_q_learning_energy.csv', 'trajectory_q_learning_pricing.csv',
        'kpis_fixed.csv', 'kpis_weather.csv', 'kpis_energy.csv', 'kpis_pricing.csv',
