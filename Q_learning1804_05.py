...
def run_experiment(schema_path: Path, building_names: list[str], episodes: int,
                   baseline_cooling: float, random_seed: int, output_dir: Path,
    ...
    # ── Q-learning pre každý reward variant ──
    ...
    for key, display_name, reward_cls in REWARD_CONFIGS:
        print(f'\nTraining Q-learning with {display_name} ({reward_cls.__name__})...', flush=True)
        env = make_env(schema_path, building_names, random_seed, reward_function=reward_cls)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description='Q-learning: 12 reward variantov (weather, energy, pricing, comfort, peak-shaving, solar, storage, ramping, time-of-use, self-sufficiency, combined, night-precharge).'
    )
    ...
    results = run_experiment(
        ...
        baseline_cooling=args.baseline_cooling, random_seed=args.random_seed,
