...
# Comfort/efficiency penalty weights (higher => stronger comfort preference)
UNMET_DEMAND_PENALTY = 1.20
OVER_SUPPLY_PENALTY = 0.25
...
class BuildingEnv:
    def __init__(self, df: pd.DataFrame):
        ...
        self.t = 0
        self.max_occupancy = max(1.0, float(self.df['occupant_count'].max()))

    def reset(self, random_start=True, start_index=None):
        if start_index is not None:
            self.t = int(max(0, min(start_index, self.n_steps - 1)))
        elif random_start:
    ...
    def step(self, action: int):
        ...
        # Comfort cost uses unmet HVAC demand and is stronger when occupancy is high.
        occ_norm = min(1.0, occupancy / self.max_occupancy)
        unmet_demand = (1.0 - intensity) * hvac_demand
        comfort_cost = UNMET_DEMAND_PENALTY * unmet_demand * (0.5 + 0.5 * occ_norm)

        # Efficiency cost for unnecessary high-intensity action at very low demand.
        if action == 2 and hvac_demand < LOW_DEMAND_THRESHOLD:
            comfort_cost += OVER_SUPPLY_PENALTY * (LOW_DEMAND_THRESHOLD - hvac_demand)

        # Reward is the negative combined objective cost.
        reward = -(total_energy + comfort_cost)

        self.t += 1
        done = (self.t >= self.n_steps - 1)

        next_state = self.get_state_info() if not done else None
        return next_state, reward, done, total_energy, comfort_cost
...
def train(env: BuildingEnv, agent: QLearningAgent, edges):
    ...
    for _ in tqdm(range(TRAIN_EPISODES), desc='Training Q-learning'):
        ...
        for _step in range(EPISODE_LENGTH):
            ...
            next_state, reward, done, step_energy, _comfort_penalty = env.step(a)
...
def evaluate(env: BuildingEnv, policy_fn, edges, n_episodes=20, start_indices=None):
    ep_energies = []
    ep_rewards = []
    ep_comfort_costs = []
    ...
    for _ in range(n_episodes):
        if start_indices is None:
            ...
            state = env.reset(random_start=False, start_index=start_indices[_])
        ep_energy = 0.0
        ep_reward = 0.0
        ep_comfort_cost = 0.0
        ...
        for _step in range(EPISODE_LENGTH):
            s = state_to_index(state, edges)
            a = policy_fn(s)
            next_state, reward, done, step_energy, comfort_cost = env.step(a)

            ep_energy += step_energy
            ep_reward += reward
            ep_comfort_cost += comfort_cost
        ...
        ep_comfort_costs.append(ep_comfort_cost)

    avg_energy = float(np.mean(ep_energies))
    avg_reward = float(np.mean(ep_rewards))
    avg_comfort_cost = float(np.mean(ep_comfort_costs))
    # What the agent optimizes: lower objective cost is better.
    objective_cost = -avg_reward

    return {
        'avg_energy': avg_energy,
        'avg_reward': avg_reward,
        'avg_comfort_cost': avg_comfort_cost,
...
def main():
    ...
    eval_rng = np.random.default_rng(RANDOM_SEED + 123)
    max_start = max(0, len(df) - EPISODE_LENGTH - 1)
    eval_starts = [int(eval_rng.integers(0, max_start + 1)) for _ in range(20)]

    q_policy = lambda s: agent.act(s, training=False)
    q_res = evaluate(env, q_policy, edges, n_episodes=20, start_indices=eval_starts)
    print(f'    Q-learning avg energy: {q_res["avg_energy"]:.4f}')
    print(f'    Q-learning avg comfort cost: {q_res["avg_comfort_cost"]:.4f}')
    print(f'    Q-learning objective cost (-reward): {q_res["objective_cost"]:.4f}')

    print('\n[5] Evaluating fixed baselines...')
    baseline_results = {}
    for a_id, a_name in ACTIONS.items():
        fixed = lambda _s, aa=a_id: aa
        res = evaluate(env, fixed, edges, n_episodes=20, start_indices=eval_starts)
        baseline_results[a_name] = res
        print(
            f'    {a_name:10s} avg energy: {res["avg_energy"]:.4f} | '
            f'comfort cost: {res["avg_comfort_cost"]:.4f} | '
            f'objective cost: {res["objective_cost"]:.4f}'
        )
    ...
    print('\n[6b] Objective comparison (energy + comfort penalties)')
    best_obj_name, best_obj_res = min(baseline_results.items(), key=lambda kv: kv[1]['objective_cost'])
    obj_gain = best_obj_res['objective_cost'] - q_res['objective_cost']
    obj_gain_pct = 100.0 * obj_gain / best_obj_res['objective_cost'] if best_obj_res['objective_cost'] > 0 else 0.0
    print('-' * 72)
    print(f'Q-learning objective cost:      {q_res["objective_cost"]:.4f}')
    print(f'Best fixed ({best_obj_name}):   {best_obj_res["objective_cost"]:.4f}')
    print(f'Objective improvement:          {obj_gain:.4f}')
    print(f'Objective improvement [%]:      {obj_gain_pct:.2f}%')
    print('-' * 72)
