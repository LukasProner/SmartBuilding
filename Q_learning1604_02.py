...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / 'outputs_q_learning1604_02'
ACTIVE_OBSERVATIONS = [
    'occupant_count',
...
def split_month_ranges(horizon: int) -> list[tuple[int, int, int]]:
    month_ranges: list[tuple[int, int, int]] = []
    start = 0

    for month_index, month_hours in enumerate(HOURS_PER_MONTH):
        if start >= horizon:
            break

        end = min(start + month_hours, horizon)
        month_ranges.append((month_index, start, end))
        start = end
...
def save_monthly_comparisons(
    ...
    max_horizon = min(len(fixed_run.trajectory), len(learned_run.trajectory))
    month_ranges = split_month_ranges(max_horizon)
    monthly_dir = output_dir / 'monthly_comparisons'
    monthly_dir.mkdir(parents=True, exist_ok=True)
    rows: list[dict[str, float | int | str]] = []

    for month_index, start, end in month_ranges:
        fixed_slice = fixed_run.trajectory.iloc[start:end].copy()
        learned_slice = learned_run.trajectory.iloc[start:end].copy()

        local_time = np.arange(end - start)
        fixed_cumulative = np.cumsum(np.asarray(fixed_slice['reward'], dtype=float))
        learned_cumulative = np.cumsum(np.asarray(learned_slice['reward'], dtype=float))
        fixed_grid_cumulative = np.cumsum(np.asarray(fixed_slice['grid_import_kwh'], dtype=float))
        learned_grid_cumulative = np.cumsum(np.asarray(learned_slice['grid_import_kwh'], dtype=float))

        fig, axes = plt.subplots(1, 2, figsize=(12, 4.2))
        axes[0].plot(local_time, fixed_cumulative, linestyle='--', linewidth=1.8, color='#9aa0a6', label='Fixed')
        axes[0].plot(local_time, learned_cumulative, linewidth=1.9, color='#1d3557', label='Q-learning')
        axes[0].set_title(f'{MONTH_NAMES[month_index]}: cumulative reward')
        axes[0].set_xlabel('Hour in month')
        axes[0].set_ylabel('Cumulative reward')
        axes[0].grid(alpha=0.25)
        axes[0].legend(loc='best', fontsize=8)

        axes[1].plot(local_time, fixed_grid_cumulative, linestyle='--', linewidth=1.8, color='#9aa0a6', label='Fixed')
        axes[1].plot(local_time, learned_grid_cumulative, linewidth=1.9, color='#1d3557', label='Q-learning')
        axes[1].set_title(f'{MONTH_NAMES[month_index]}: cumulative grid import')
        axes[1].set_xlabel('Hour in month')
        axes[1].set_ylabel('kWh')
        axes[1].grid(alpha=0.25)
        axes[1].legend(loc='best', fontsize=8)

        fig.suptitle(f'Monthly comparison - {MONTH_NAMES[month_index]}', fontsize=12)
        fig.tight_layout()
        fig.savefig(monthly_dir / f'{month_index + 1:02d}_{MONTH_NAMES[month_index].lower()}_comparison.png', dpi=160)
        plt.close(fig)

        fixed_grid = float(np.sum(np.asarray(fixed_slice['grid_import_kwh'], dtype=float)))
        learned_grid = float(np.sum(np.asarray(learned_slice['grid_import_kwh'], dtype=float)))
        savings_pct = None
        if fixed_grid > 0.0:
            savings_pct = 100.0 * (fixed_grid - learned_grid) / fixed_grid
        ...
        rows.append(
            {
                ...
                'hours': end - start,
                'fixed_cumulative_reward': float(np.sum(np.asarray(fixed_slice['reward'], dtype=float))),
                'q_learning_cumulative_reward': float(np.sum(np.asarray(learned_slice['reward'], dtype=float))),
                'fixed_grid_import_kwh': fixed_grid,
                'q_learning_grid_import_kwh': learned_grid,
                'q_learning_savings_vs_fixed_pct': savings_pct,
            }
        )
...
def main() -> None:
    ...
    parser.add_argument('--episodes', type=int, default=700, help='Number of training episodes.')
    ...
    print(f'  {args.output_dir / "monthly_summary.csv"}', flush=True)
    print(f'  {args.output_dir / "monthly_comparisons"}', flush=True)
